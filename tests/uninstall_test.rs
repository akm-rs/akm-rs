...
fn install_fixture(paths: &Paths) {
    ...
    // Cold library with one core skill
    ...
    std::fs::create_dir_all(paths.cache_dir().join("skills-community-registry")).unwrap();

    // User content in ~/.akm
    let artifacts = paths.default_artifacts_dir().join("my-repo");
    std::fs::create_dir_all(&artifacts).unwrap();
    std::fs::write(artifacts.join("note.md"), "keep me").unwrap();
    std::fs::write(paths.global_instructions(), "be nice").unwrap();

    // Global symlink in a tool dir (what rebuild_core produces)
    let claude_skills = paths.home().join(".claude").join("skills");
    std::fs::create_dir_all(&claude_skills).unwrap();
    std::os::unix::fs::symlink(&skill_dir, claude_skills.join("tdd")).unwrap();
}
...
fn uninstall_default_preserves_user_content() {
    ...
    assert!(paths
        ...
        .is_file());
    assert!(paths.global_instructions().is_file());
}
...
fn uninstall_purge_removes_everything() {
    ...
    assert!(
        ...
        "data dir should be fully removed"
    );
...
fn shell_init_returns_early_when_disabled() {
    ...
    let out = std::process::Command::new("bash")
        .arg("-c")
        .arg(format!(
            "source '{}' && declare -F claude copilot opencode; echo \"defined=$?\"",
...
fn shell_init_defines_wrappers_when_enabled() {
    ...
    let out = std::process::Command::new("bash")
        .arg("-c")
        .arg(format!(
            "source '{}' && declare -F claude copilot opencode >/dev/null && echo defined=0",
            paths.shell_init().display()
        ))
        .env("XDG_CONFIG_HOME", tmp.path().join("config"))
        .output()
