...
//! global instructions (`~/.akm/global-instructions.md`), and the cold
//! library (`$XDG_DATA_HOME/akm/{skills,agents}`). `--purge` removes
//! everything.
//!
//! Instruction files distributed into tool dirs (e.g. `~/.claude/CLAUDE.md`)
//! are never touched — they may contain user edits that drifted from
//! `global-instructions.md`, so deleting them could destroy user content.
//!
...
pub fn run(
    ...
    if !purge {
        ...
        println!("  {}", paths.global_instructions().display());
        println!(
            "  {} and {}",
    ...
    println!("Restart your shell to drop the claude/copilot/opencode wrappers.");
    Ok(())
}
...
fn print_plan(paths: &Paths, purge: bool) {
    ...
    if purge {
        println!(
            "  - data:   {} (including cold library)",
            paths.data_dir().display()
        );
        println!("  - {} (including artifacts)", paths.akm_home().display());
    } else {
        println!(
            "  - data:   {} (library.json, tools.json, shell/)",
            paths.data_dir().display()
        );
        println!();
        println!("Preserved:");
        ...
        println!(
            "  - global instructions: {}",
            paths.global_instructions().display()
        );
        println!(
            "  - cold library: {} and {}",
            paths.skills_dir().display(),
...
pub fn remove_files(paths: &Paths, tool_dirs: &ToolDirs, purge: bool) -> Result<()> {
    ...
    // 5. Data dir: everything, or everything except the cold library
    if purge {
