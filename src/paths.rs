...
impl Paths {
    ...
    /// `$XDG_DATA_HOME/akm/library/` — the git working tree of the personal
    /// registry, and the cold library itself.
    ///
    /// The registry is checked out here rather than copied out of a cache, so
    /// git's own metadata answers "what changed locally", "what changed
    /// remotely" and "how do I get back to the last synced state". AKM-owned
    /// files (`shell/`, `tools.json`, `local.json`) deliberately live one level
    /// up, outside the repository.
    pub fn library_dir(&self) -> PathBuf {
        self.data_dir.join("library")
    }

    /// `$XDG_DATA_HOME/akm/library.json` — the derived spec index.
    ///
    /// Outside the registry tree, alongside `local.json` and `tools.json`, for
    /// the same reason they are: libgen regenerates it from the specs on disk
    /// on every sync, and [`crate::library::local::LocalOverrides::apply`] then
    /// folds this machine's core deviations into it. It is derived *and*
    /// machine-local, so it can neither conflict nor be committed by a stray
    /// `git add -A` in the working tree.
    pub fn library_json(&self) -> PathBuf {
        self.data_dir.join("library.json")
    }

    /// `$XDG_DATA_HOME/akm/local.json` — machine-local core deviations.
    ///
    /// Outside the registry tree: these toggles are the one piece of state that
    /// must not propagate to other machines.
    ...
    /// `$XDG_DATA_HOME/akm/library/skills/` — installed skills
    pub fn skills_dir(&self) -> PathBuf {
        self.library_dir().join("skills")
    }

    /// `$XDG_DATA_HOME/akm/library/agents/` — installed agents
    pub fn agents_dir(&self) -> PathBuf {
        self.library_dir().join("agents")
    }

    /// `$XDG_DATA_HOME/akm/library/instructions/global.md`
    ///
    /// Global instructions live in the registry so they propagate between
    /// machines through the same clone, drift and publish flow as specs.
    pub fn instructions_file(&self) -> PathBuf {
        self.library_dir().join("instructions").join("global.md")
    }
    ...
    /// `$HOME/.akm/global-instructions.md` — the pre-rc4 location.
    ///
    /// Instructions now live in the registry ([`Paths::instructions_file`]).
    /// This path is only consulted to seed that file on first sync, so a
    /// machine upgrading from an earlier release keeps what it had written.
    pub fn legacy_global_instructions(&self) -> PathBuf {
