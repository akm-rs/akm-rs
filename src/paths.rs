...
impl Paths {
    ...
    /// `$XDG_DATA_HOME/akm/library/library.json`
    ///
    /// A derived index, gitignored inside the registry: it is regenerated by
    /// libgen from the specs on disk and their `akm.json` sidecars, so it can
    /// never be the thing that conflicts.
    pub fn library_json(&self) -> PathBuf {
        self.library_dir().join("library.json")
    }
