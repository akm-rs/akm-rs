...
# and LLM tool wrappers (claude, copilot, opencode, pi, vibe).
#
...
# (claude, copilot, opencode, pi, vibe) stay vanilla. Re-enable with `akm enable`.
...
_akm_skills_session_start() {
  ...
  # Create staging directory structure. Pi and Vibe get an agents/ subdir they
  # never read, so the layout matches the session symlinks, which cover both.
  local tool_dir
  for tool_dir in .claude .copilot .agents .pi .vibe; do
...
_akm_session_start() {
  ...
  if [[ -n "$artifact_dir" && -n "$staging_dir" ]]; then
    ...
    local tool_dir
    for tool_dir in .claude .copilot .agents .vibe; do
...
# Pi's management subcommands must be the *first* argument — anything before
# them is parsed as options and the subcommand itself becomes the opening chat
# message. They start no agent, so the wrapper stays out of their way entirely.
_akm_pi_is_subcommand() {
  case "$1" in
    install|remove|uninstall|update|list|config) return 0 ;;
    *) return 1 ;;
  esac
}
...
_akm_wrap_tool() {
  ...
  if [[ "$tool" == "pi" ]] && _akm_pi_is_subcommand "${1:-}"; then
    command pi "$@"
    return $?
  fi
  ...
  case "$tool" in
    ...
    claude)
      ...
      ;;
    copilot|vibe)
      # Both support --add-dir but have no --append-system-prompt, so they
      # rely on the staging README and the read-only root instead. Artifacts
      # are symlinked into the staging dir, so one dir is enough; with
      # artifacts but no staging dir, pass the artifact dir directly.
      # Vibe's --add-dir (2.10.0+) pulls in <dir>/.vibe/skills. Vibe has no
      # argv[1] subcommands, so nothing needs the Pi-style pass-through.
      if [[ -n "${_AKM_STAGING_DIR:-}" ]]; then
        cmd+=(--add-dir "$_AKM_STAGING_DIR")
      elif [[ -n "${_AKM_ARTIFACT_DIR:-}" ]]; then
        cmd+=(--add-dir "$_AKM_ARTIFACT_DIR")
